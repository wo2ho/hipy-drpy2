...
4. 本人将路由器设置了每天早上 6 点重启，使得《[四](https://proxy-tutorials.dustinwin.cc.cd/posts/share-shellcrash-mihomo-geodata/#%E5%9B%9B-%E6%B7%BB%E5%8A%A0%E5%AE%9A%E6%97%B6%E4%BB%BB%E5%8A%A1)》中设置的定时任务生效
...
## 二、 导入路由规则文件和 [CN_IP 文件](https://github.com/DustinWin/geoip)
> 路由规则文件和 user.yaml 都属高度定制，牵一发而动全身
{: .prompt-warning }
连接 SSH 后执行如下命令：

```shell
curl -fo "${CRASHDIR}/GeoSite.dat" -L https://cdn.jsdelivr.net/gh/DustinWin/ruleset_geodata@mihomo-geodata/geosite.dat
curl -fo "${CRASHDIR}/GeoIP.dat" -L https://cdn.jsdelivr.net/gh/DustinWin/ruleset_geodata@mihomo-geodata/geoip-lite.dat
curl -fo "${CRASHDIR}/Country.mmdb" -L https://cdn.jsdelivr.net/gh/DustinWin/ruleset_geodata@mihomo-geodata/Country-lite.mmdb
curl -fo "${CRASHDIR}/cn_ip.txt" -L https://cdn.jsdelivr.net/gh/DustinWin/geoip@ips/cn_ipv4.txt
curl -fo "${CRASHDIR}/cn_ipv6.txt" -L https://cdn.jsdelivr.net/gh/DustinWin/geoip@ips/cn_ipv6.txt
```
...
注：
- ① 本 `dns` 配置中，国内域名走国内 DNS 解析，国外域名走 `fake-ip`，未知域名在匹配 `GEOIP:cn` 规则时会由国外 DNS 解析且配置 `ecs`，解析出 IP 在国内则走 `国内 IP` 规则，否则走 `漏网之鱼` 规则（有效解决了“心理 DNS 泄露问题”，详见《[搭载 mihomo 内核配置 DNS 不泄露教程-geodata 方案](https://proxy-tutorials.dustinwin.cc.cd/posts/dnsnoleaks-mihomo-geodata/)》）
...
## 四、 添加定时任务
可参考《[ShellCrash 搭载 mihomo 内核的配置-geodata 方案/添加定时任务](https://proxy-tutorials.dustinwin.cc.cd/posts/toolsettings-shellcrash-mihomo-geodata/#%E4%BA%8C-%E6%B7%BB%E5%8A%A0%E5%AE%9A%E6%97%B6%E4%BB%BB%E5%8A%A1)》

## 五、 设置部分
1. 设置可参考《[ShellCrash 搭载 mihomo 内核的配置-geodata 方案/设置部分](https://proxy-tutorials.dustinwin.cc.cd/posts/toolsettings-shellcrash-mihomo-geodata/#%E5%9B%9B-%E6%B7%BB%E5%8A%A0%E5%AE%9A%E6%97%B6%E4%BB%BB%E5%8A%A1)》，此处只列举配置的不同之处
